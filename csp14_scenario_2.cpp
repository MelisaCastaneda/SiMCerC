...
int ReflexCytologyTest( int GP )									// Perform Cytology test
{
	double rnd1 = Random() ;
	double rnd2 = Random() ; 
	int result = 0 ;
	if ( GP )
		costs += 27.05 ;                                       		// costs of Reflex Cytology test
	else
		costs += 0 ;
	result = (  ( ( CurrentState >= 3 ) && ( rnd1 < 0.782 ) ) ) ;  	// sensitivities for Reflex Cytology test
	if ( !result )
	{
		if ( rnd2 > 0.804 )											// false positive detected
		{
			result = 1;
			NFalsePositive++ ;
		}
...
int ControlCytologyTest( void )										// Perform Control Cytology
{
	...
	if ( !result )
	{
		if ( rnd2 > 0.80 )											// false positive detected
		{
			result = 1;
			NFalsePositive++ ;
		}
	...
	if ( ScreenAge() )
	{
		...
		if ( Compliance() ) 											// check whether woman complies to screening
		{
			...
			if ( Random() < 0.91 )										// 91% goes to GP, 9% does self-test
			{
				...
				{
					if (debug)printf("4");
					CytologyTest = ReflexCytologyTest( 1 ) ;
				}
				else 
				{
					if (debug)printf("5");
			...
			if ( !stop )
			{
				...
				else													// indirect referral
				{
					...
					{
						if (debug) printf("e");
						CytologyTest = ControlCytologyTest() ;
						if (CytologyTest)
						{
							if (debug) printf("h");
							stop = ( Random() > 0.58 ) ;				// participate in confirmatory diagnosis?
							if ((debug) && (stop)) printf("i");
							if ((debug) && (!stop))printf("j");
							if (!stop)
								DetReferral = 2 ;
						}
						else
...
int main()
{
	...
	printf( "Cervix Screening Program CSP - scenario_II\n" ) ;
	printf( "Test version 14_2 - 31 mar 2023\n" ) ;
	printf( "Number of women: %d\n", Nwomen ) ;
	printf( "For every iteration i:\n" ) ;
	printf( "csp14_2i.out: for every woman: state per age\n" ) ;
	printf( "csp14_2i.det: for every woman: screening age, participate, referral, state, FIGO, tumor death, death age, costs\n" ) ;
