		...
		if ( Compliance() ) 											// check whether woman complies to screening
		{
			...
			else
			{
				...
				if ( HPVtest ) 
				{
					...
					{
						if (debug)printf("8");
						CytologyTest = ReflexCytologyTest( 0 ) ;
					}
					else
					{
						if (debug)printf("9");	
...
int main()
{
	...
	printf( "Cervix Screening Program CSP - base case scenario\n" ) ;
	printf( "Test version 14_b - 31 mar 2023\n" ) ;
	printf( "Number of women: %d\n", Nwomen ) ;
	printf( "For every iteration i:\n" ) ;
	printf( "csp13_bi.out: for every woman: state per age\n" ) ;
	printf( "csp13_bi.det: for every woman: screening age, participate, referral, state, FIGO, tumor death, death age, costs\n" ) ;
